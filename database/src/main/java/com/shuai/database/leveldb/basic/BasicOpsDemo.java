package com.shuai.database.leveldb.basic;
...
public class BasicOpsDemo {
    ...
    private static final String DB_PATH = "target/leveldb-demo";
    ...
    public static void run() throws IOException {
        ...
        try (LevelDbTemplate template = new LevelDbTemplate(config)) {
            ...
            // [1] 写入数据
            System.out.println("\n[1] 写入数据 (put)");
            template.put("key1", "Hello LevelDB!");
            template.put("key2", "Key-Value Store");
            template.put("key3", "Embedded Database");
            System.out.println("  put(key1, 'Hello LevelDB!')");
